...
class FESS(EnergyStorageDevice):   # 飞轮储能
    def __init__(self):
        ...
        # 修正内阻：使压降约50V（额定电流12500A，0.004Ω）
...
class SC(EnergyStorageDevice):   # 超级电容器（改进丰富模型）
    def __init__(self):
        ...
        # 修正电容计算：基于能量差 C = 2E / (Vmax^2 - Vmin^2)
        ...
        self.ESR = 0.001               # 欧姆，修正为更合理的值（压降约24V）
    ...
    def _calc_electrical(self, dt):
        ...
        # 2. 根据当前功率估算电流（用V_oc近似，避免迭代）
        if abs(self.current_power) > 1e-6:
            # 先假设端电压约等于V_oc估算电流
            i_est = self.current_power * 1e6 / v_oc
        else:
            i_est = 0.0

        # 3. 温度对ESR的影响
        ESR_actual = self.ESR * self.aging_factor_ESR * (1 + self.temp_coeff_ESR * (self.temperature - 25))

        # 4. 考虑自放电（通过EPR产生的漏电流）
        i_leak = v_oc / self.EPR        # 漏电流，始终存在
        # 由于漏电流很小，可忽略其对功率的影响，但可加到电流上作为测量值的一部分

        # 5. 端电压计算（考虑ESR压降）
        if self.current_power > 0:      # 放电
            v_term = v_oc - i_est * ESR_actual
        elif self.current_power < 0:    # 充电
            v_term = v_oc + i_est * ESR_actual
        else:
            v_term = v_oc
        ...
        # 加入一些高频噪声模拟真实波动（幅值较小）
        noise_voltage = np.random.normal(0, 2.0)   # 2V噪声
        noise_current = np.random.normal(0, 5.0)   # 5A噪声
        v_term += noise_voltage
        i_est += noise_current

        # 钳位电压电流
        self.voltage = max(0, v_term)
        self.current = i_est

        # 可选：模拟温度缓慢变化（例如随充放电发热）
        # 简单假设：功率大时温度升高
        self.temperature += 0.001 * abs(self.current_power) * dt

        # 老化模拟：根据累积放电深度增加循环次数，这里简化处理
        # 每仿真1秒，如果功率不为零，增加一点点循环计数
        ...
        # 低通滤波（保留更多细节，alpha调大）
        self._apply_lowpass_filter(dt, alpha=0.3)
...
class BESS(EnergyStorageDevice):   # 锂电池储能
    def __init__(self):
        ...
        # 修正内阻：使压降约49V（额定电流3750A，0.013Ω）
        self.internal_resistance = 0.013
...
class CAES(EnergyStorageDevice):   # 压缩空气储能
    def __init__(self):
        ...
        # 修正内阻：使压降约1000V（额定电流50A，20Ω）
        self.internal_resistance = 20.0
...
class HierarchicalMPC:
        ...
        if freq_type == 'high':
            ...
            for dev in eligible:
                ...
                if remaining > 0:
                    ...
                    assign = max(remaining, charge_limit)   # remaining为负，charge_limit为负，取较大者（即绝对值较小）
                if abs(assign) > 1e-6:
                    allocation[dev.name] = allocation.get(dev.name, 0.0) + assign
                    remaining -= assign
    ...
    def real_time_balance(self, devices, net_load, dt):
        ...
        # 修正：增加缓冲区长度至200，以覆盖20s（dt=0.1时）
        if self._buffer is None:
            self._buffer = np.ones(200) * net_load
        else:
...
class HybridEnergyStorageSystem:
    ...
    def simulate(self, pulse_amp, pulse_dur, plant_power=10.0, sim_time=30, dt=0.1, econ_interval=1, eco_mode=False):
        ...
        for t in range(n):
            ...
            # 上层经济调度 - 修正：使用取模计数器确保每 econ_interval 步执行一次，第一次在t=0执行
            if econ_cnt == 0:
            ...
            if t % 5 == 0:
                self._update_monitor(total_purchase + total_op + total_aging + total_fuel + total_carbon_penalty,
                                     res['grid_power'][t])
...
class HESSGUI:
    ...
    def plot_scenarios(self, scenarios):
        ...
        plt.scatter(x, scenarios, marker='o', color='b', label='场景值')
        plt.xlabel('场景编号')
        plt.ylabel('净负荷 (MW)')
    ...
    def plot_all(self, hess, res):
        ...
        try:
            ...
            plt.subplot(4, 4, 3)
            for dev in hess.devices:
                current_smooth = gaussian_filter1d(res['device_current'][dev.name], sigma=0.5)
