...
class HESSGUI:
    ...
    def show_results(self, hess, res, amp, dur, selected):
        ...
        if hess.mpc.stochastic and hess.mpc.last_scenarios is not None:
            ttk.Button(btn_frame, text="🎲 显示随机规划场景", command=lambda: self.plot_scenarios(hess.mpc.last_scenarios), width=20).pack(side='left', padx=5)
    ...
    def plot_all(self, hess, res):
        ...
        try:
            ...
            plt.plot(res['time'], res['smoothed_load'], 'b--', lw=2, label='平滑后负荷(电厂+储能)')
            ...
            plt.title('脉冲平滑效果对比', fontsize=12)
            ...
            plt.title('各设备电流变化 (超强滤波)', fontsize=12)
            ...
            plt.title('各设备电压变化 (超强滤波)', fontsize=12)
            plt.legend(fontsize=7, loc='upper right')
            plt.grid(True, alpha=0.3)
