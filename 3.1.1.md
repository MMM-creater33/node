...
class EnergyStorageDevice:
    ...
    def update_state(self, power_set, dt):
        ...
        # 能量更新
        if power >= 0:  # 放电
            # 修正：放电时应除以效率，以反映内部消耗的能量
            energy_out = power * dt / 3600 / self.discharge_eff  # <-- 修正点（原为 * discharge_eff）
...
class HESSGUI:
    ...
    def show_results(self, hess, res, amp, dur, selected):
        ...
        # 脉冲期间功率分配占比
        mask = (res['time'] >= 100) & (res['time'] < 100 + dur)
        pulse_power = {}
        for dev in hess.devices:
    ...
    def plot_all(self, hess, res):
        ...
        try:
            ...
            # 2. 脉冲期间功率分配饼图
            ax = axes[1]
            mask = (res['time'] >= 100) & (res['time'] < 100 + res['pulse_dur'])
            pulse_power = {}
            for dev in hess.devices:
                avg = np.mean(np.abs(res['device_power'][dev.name][mask]))
                if avg > 0.05:
            ...
            # 16. 脉冲局部放大
            ax = axes[15]
            mask_local = (res['time'] >= 98) & (res['time'] <= 102 + res['pulse_dur'])
