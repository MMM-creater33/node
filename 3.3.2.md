...
# ==================== 二、五类储能设备具体模型 ====================
# 重新分配功率与容量，使能量型设备（锂电池、压缩空气）承担主要功率
# 总功率20MW / 总容量480MWh
# ------------------------------------------------------------------------------
# 设备名称       功率(MW)  容量(MWh)  响应时间(s)  老化系数(元/MW^1.8/h)  说明
# ------------------------------------------------------------------------------
# 超级电容器     2.0       8          0.2         0.6                   高频，小功率
# 飞轮储能       2.0       6          0.3         1.2                   高频/中频，小功率
# 超导磁储能     1.0       4          0.2         0.8                   极端高频，小功率
# 锂电池储能     10.0      300        0.5         3.5                   中/低频，大功率能量型
# 压缩空气储能   5.0       162        1.0         0.5                   低频，大功率能量型
# ------------------------------------------------------------------------------
...
class FESS(EnergyStorageDevice):  # 飞轮储能
    def __init__(self):
        ...
        # 内阻调整：保持额定压降约50V（原12500A*0.004=50V，新4167A*0.012=50V）
        self.internal_resistance = 0.012
...
class SMES(EnergyStorageDevice):  # 超导磁储能
    def __init__(self):
        ...
        # 临界电流设为3000A（原10000A，新额定电流1667A，留余量）
        self.critical_current = 3000
        # 内阻调整：保持额定压降约66.7V（原6667A*0.01=66.7V，新1667A*0.04=66.7V）
        self.internal_resistance = 0.04
...
class SC(EnergyStorageDevice):  # 超级电容器
    def __init__(self):
        ...
        # 内阻调整：保持额定压降约33V（原33333A*0.001=33V，新8333A*0.004=33V）
        self.ESR_base = 0.004
        self.EPR = 1000.0
...
class BESS(EnergyStorageDevice):  # 锂电池储能
    def __init__(self):
        ...
        # 内阻调整：保持额定压降约48.75V（原3750A*0.013=48.75V，新25000A*0.002=50V，接近）
        self.R0 = 0.002
...
class CAES(EnergyStorageDevice):  # 压缩空气储能
    def __init__(self):
        ...
        # 内阻调整：保持额定压降约1000V（原50A*20=1000V，新500A*2=1000V）
        self.R0 = 2.0
        self.p0 = 1.0
        self.k = 1.4
...
class HESSGUI:
    ...
    def setup_ui(self):
        ...
        ttk.Label(advf, text="碳价格: 0.05 元/kg CO₂ (固定)",
                  font=('微软雅黑', 9), foreground=COLOR_TEXT_SECONDARY).pack(anchor='w', padx=20)
    ...
    def show_results(self, hess, res, amp, dur, selected):
        ...
        text.insert('end', "  • 所有电气量均为瞬时计算值，未添加任何人为滤波，完全符合物理规律。\n")
        text.insert('end', "\n")
    ...
    def plot_all(self, hess, res):
        try:
            ...
            for dev in hess.devices:
                ...
                ax.plot(t_down, current_down, label=dev.name, linewidth=1.5)
            ax.set_xlabel('时间 (秒)', fontsize=10)
            ax.set_ylabel('电流 (A)', fontsize=10)
            ax.set_title('各设备电流变化（无滤波）', fontsize=12)
            ...
            for dev in hess.devices:
                ...
                ax.plot(t_down, voltage_down, label=dev.name, linewidth=1.5)
            ax.set_xlabel('时间 (秒)', fontsize=10)
            ax.set_ylabel('电压 (V)', fontsize=10)
            ax.set_title('各设备电压变化（无滤波）', fontsize=12)
            ax.legend(fontsize=7, loc='upper right')
            ax.grid(True, alpha=0.3)
            ...
            if len(handles) > 8:
                ax.legend(handles[:8], labels[:8], fontsize=5, loc='upper right', ncol=2)
            else:
            ...
            ax.plot(res['time'][mask_local], res['smoothed_load'][mask_local], 'b--', lw=2, label='电厂+储能总出力')
            ax.set_xlabel('时间 (秒)', fontsize=10)
            ax.set_ylabel('功率 (MW)', fontsize=10)
...
def main():
    if not PYWT_AVAILABLE:
        print("⚠️ PyWavelets 未安装，小波分解使用改进的移动平均近似。")
    app = HESSGUI()
    app.root.mainloop()
