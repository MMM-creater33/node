...
class FESS(EnergyStorageDevice):  # 飞轮储能
    def __init__(self):
        ...
        self.internal_resistance = 0.01  # 欧姆，增大内阻使电压变化明显
...
class SMES(EnergyStorageDevice):  # 超导磁储能
    def __init__(self):
        ...
        self.internal_resistance = 0.01  # 欧姆
...
class SC(EnergyStorageDevice):  # 超级电容器
    def __init__(self):
        ...
        self.internal_resistance = 0.02  # 欧姆，增大内阻使放电电压下降明显
...
class BESS(EnergyStorageDevice):  # 锂电池储能
    def __init__(self):
        ...
        self.internal_resistance = 0.005  # 欧姆
...
class CAES(EnergyStorageDevice):  # 压缩空气储能
    def __init__(self):
        ...
        self.internal_resistance = 5.0  # 欧姆，较高因为电压高
    ...
    def _calc_electrical(self, dt):
        if abs(self.current_power) > 1e-3:
            ...
            self.current = 0
        # 开路电压随SOC变化（模拟储气压力影响），范围0.6~1.4倍额定电压，变化更明显
        v_oc = self.voltage_nominal * (0.6 + 0.8 * self.soc)
        vdrop = self.current * self.internal_resistance
        if self.current_power > 0:
            self.voltage = v_oc - vdrop
        elif self.current_power < 0:
